...
func TestPruneToVerifyIncremental(t *testing.T) {
	...
	for _, c := range testCases {
		prunedOp := PruneToVerifyIncremental(c.index, c.version)
		assert.Equalf(t, c.expectedOp, prunedOp, "The pruned operation should match for test case with index %d and version %d", c.index, c.version)
	}
...
func BenchmarkPruneToVerifyIncremental(b *testing.B) {

	log.SetLogger("BenchmarkPruneToVerifyIncremental", log.SILENT)

	b.ResetTimer()
	for i := uint64(0); i < uint64(b.N); i++ {
		pruned := PruneToVerifyIncremental(0, i)
		assert.NotNil(b, pruned)
	}
