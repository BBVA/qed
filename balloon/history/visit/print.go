...
func (v *PrintVisitor) VisitNode(pos *navigation.Position) {
	v.tokens = append(v.tokens, fmt.Sprintf("%sNode(%v)", v.indent(pos.Height), pos))
}

func (v *PrintVisitor) VisitPartialNode(pos *navigation.Position) {
	v.tokens = append(v.tokens, fmt.Sprintf("%sPartialNode(%v)", v.indent(pos.Height), pos))
}

func (v *PrintVisitor) VisitLeaf(pos *navigation.Position, value []byte) {
	v.tokens = append(v.tokens, fmt.Sprintf("%sLeaf(%v)[%x]", v.indent(pos.Height), pos, value))
}

func (v *PrintVisitor) VisitCached(pos *navigation.Position, cachedDigest hashing.Digest) {
	v.tokens = append(v.tokens, fmt.Sprintf("%sCached(%v)[%x]", v.indent(pos.Height), pos, cachedDigest))
}

func (v *PrintVisitor) VisitCollectable(pos *navigation.Position) {
	v.tokens = append(v.tokens, fmt.Sprintf("%sCollectable(%v)", v.indent(pos.Height), pos))
}

func (v *PrintVisitor) VisitMutable(pos *navigation.Position) {
	v.tokens = append(v.tokens, fmt.Sprintf("%sMutable(%v)", v.indent(pos.Height), pos))
}

func (v *PrintVisitor) VisitCacheable(pos *navigation.Position) {
